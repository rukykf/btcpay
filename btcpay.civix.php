 ...
 * @return bool
 */
function _btcpay_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _btcpay_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _btcpay_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _btcpay_civix_fixNavigationMenu($nodes);
  }
}
...
function _btcpay_civix_fixNavigationMenu(&$nodes) {
  ...
  _btcpay_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _btcpay_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
